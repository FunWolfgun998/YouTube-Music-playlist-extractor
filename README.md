# YouTube Music playlist extractor
This script return a txt/csv/json file with all the infos songs from a YouTube Music playlist.<br>
It can return: name of the song, name of the artist, name of the album, link of the song, id of the song.

## How to use:
- Visit a Playlist's page on ytm (URL looks like: `https://music.youtube.com/playlist?list=...`
- Scroll down to the bottom to load all songs of the playlist.
- Press `ctrl` + `shift` + `j` or `F12` to open the Developer Console.
- Copy the script in this repo (That's in `YouTube-Music-playlist-extractor.js`)
- Paste the code into the Developer Console on the ytm Tab, hit enter.
- Answer the questions in the pop-up windows.
- Great, you're done!

Star :star: this repo if it was useful. Follow my GitHub Profile if you appreciated this little project.

[![Star Badge](https://img.shields.io/badge/-%40FunWolfgun998-purple?style=for-the-badge&logo=github&logoColor=black&label=Follow&link=https%3A%2F%2Fgithub.com%2FunWolfgun998)](https://github.com/FunWolfgun998)


---

<div align="center">
    <p>FunWolfgun998© 2025</p>
</div>
